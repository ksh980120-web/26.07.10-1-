...
export default function AnonymousPrayerPanel({
  ...
  const startEditing = (p: AnonymousPrayer) => {
    ...
    setEditIsAnonymous(p.isAnonymous ?? true);
  ...
  const saveEdit = () => {
    ...
    if (onUpdatePrayer) {
      onUpdatePrayer(editingId!, {
        ...
        authorName: editIsAnonymous ? undefined : editAuthorName.trim()
      });
    }
    setEditingId(null);
  };
    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="bg-gradient-to-br from-[#8A9A5B]/10 to-transparent border border-[#8A9A5B]/20 rounded-3xl p-5 sm:p-6 shadow-2xs space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-[#5A5A40] uppercase tracking-wider">중보기도함</h3>
        </div>
                  ...
                  <div className="space-y-3 pt-3 animate-fadeIn">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <div>
                          <label className="block text-[9px] font-bold text-[#7A7A6A] mb-0.5">익명 여부</label>
                          <select
                            value={editIsAnonymous ? "true" : "false"}
                            onChange={(e) => setEditIsAnonymous(e.target.value === "true")}
                            className="w-full text-xs p-1.5 rounded-lg border border-[#E9E3D8] bg-white text-[#4A4A4A] focus:outline-none font-bold"
                          >
                            <option value="true">익명</option>
                            <option value="false">실명</option>
                          </select>
                        </div>
                      </div>
                      ...
                      {!editIsAnonymous && (
                        <div className="animate-fadeIn">
                          <label className="block text-[9px] font-bold text-[#7A7A6A] mb-0.5">작성자 성함</label>
                          <input
                            type="text"
                            required={!editIsAnonymous}
                            maxLength={15}
                            placeholder="성함과 직분을 입력해 주세요"
                          ...
                          {isOwner && (
                            <div className="flex items-center gap-1 border-r border-[#F0ECE4] pr-2">
                              ...
                              {onDeletePrayer && (
                                <button
                                  onClick={() => {
                                    if (confirm('이 기도제목을 삭제하시겠습니까? (삭제된 기도는 복구할 수 없습니다.)')) {
                                      onDeletePrayer(prayer.id);
                                    }
                                  }}
                                  className="p-1 text-red-600 hover:text-red-800 bg-red-50 hover:bg-red-100 border border-red-200 rounded-md transition cursor-pointer flex items-center gap-0.5 text-[9px] font-bold"
                                  title="이 기도 삭제하기"
                                >
